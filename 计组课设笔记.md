...
，其指令名后缀为.H、.S、.D、.W、.WU 分别表示该指令操作的数据类型
是半精度浮点数、单精度浮点数、双精度浮点数、有符号字、无符号字。

目的操作数rd 源操作数rj rk

指令“MULW.D.WU rd, rj, rk”中.D 对应目的操作数 rd，.WU 对应源操作数 rj 和
rk，表明这个乘法是将两个无符号字相乘，得到的双字结果写入 rd 中

指令“CRC.W.B.W rd, rj, rk中第一个.W 对应 rd，.B 对应 rj，第二个.W 对应 rk，表明这个 CRC 校验操作是将 rj 中的字节消息与 rk 中

32 位原校验值经生成新的 32 位校验值结果写入到 rd 中。
...
PC 
PC 的输出将送到虚实地址转换部件进行转换。

寄存器两个目的地址中读两个数据，作为源操作数，在alu里面加一下，作为目的操作数存回寄存器